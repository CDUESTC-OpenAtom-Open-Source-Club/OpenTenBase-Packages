...
|------|:---:|:---:|:---:|:---:|:---:|:---:|
| **v5.0** | ✅ 42 个 | ✅ 8+ 个 | ✅ | v5.0-p3 | ✅ 14/14 发行版 | ✅ 可安装 |
| **v2.6.0** | ✅ 42 个 | ✅ 8+ 个 | ✅ | v5.0-p3 | ❌ 未测试 | ❌ 未索引 |
| **v2.5.0** | ✅ 42 个 | ✅ 8+ 个 | ✅ | v5.0-p3 | ❌ 未测试 | ❌ 未索引 |

> **更新于 2026-05-31**：三个版本的 DEB/RPM 包已全部构建并发布到 v5.0-p3 release（共 154 个资产）。
> 但 APT/RPM 仓库只索引 v5.0，v2.6.0/v2.5.0 包无法通过 `apt install` 安装。
...
- [ ] APT 仓库可安装三个版本 — **根因**：`build-repo.sh` 的 `dpkg-scanpackages` 只索引 v5.0（三个版本包名相同，需要独立包名 `opentenbase-2.6.0`）
- [ ] RPM 仓库可安装三个版本 — 同上
- [ ] opentenbase-switch-version 可切换三个版本 — 依赖独立包名
- [ ] v2.6.0/v2.5.0 CI 测试 — `test-all.yml` 只下载 v5.0 包测试（`grep "_5\.0-"`）
