...
configure_repo() {
    ...
    # Check if repo has signed Release files
    local repo_signed=false
    local inrelease_url="${APT_REPO_URL}/dists/${CODENAME}/InRelease"
    local release_gpg_url="${APT_REPO_URL}/dists/${CODENAME}/Release.gpg"
    if curl -sLf --connect-timeout 5 --max-time 10 "$inrelease_url" -o /dev/null 2>/dev/null || \
       curl -sLf --connect-timeout 5 --max-time 10 "$release_gpg_url" -o /dev/null 2>/dev/null; then
        repo_signed=true
    fi

    # 写入新的源配置
    # Always configure all components to enable multi-version installation
    # main = v5.0 (latest), v2.6 = v2.6.0, v2.5 = v2.5.0
    local ALL_COMPONENTS="main v2.6 v2.5"

    if [ "$repo_signed" = "true" ]; then
        cat > "$SOURCES_LIST" << EOF
# OpenTenBase APT Repository (all versions)
# Generated by setup-apt.sh on $(date -u '+%Y-%m-%d %H:%M:%S UTC')
# Components: main (v5.0), v2.6 (v2.6.0), v2.5 (v2.5.0)
deb [signed-by=$KEYRING_PATH] ${APT_REPO_URL} $CODENAME $ALL_COMPONENTS
EOF
    else
        log_warn "仓库未签名，使用 trusted 模式"
        cat > "$SOURCES_LIST" << EOF
# OpenTenBase APT Repository (all versions)
# Generated by setup-apt.sh on $(date -u '+%Y-%m-%d %H:%M:%S UTC')
# Components: main (v5.0), v2.6 (v2.6.0), v2.5 (v2.5.0)
# NOTE: Repository is currently unsigned, using trusted mode
deb [trusted=yes] ${APT_REPO_URL} $CODENAME $ALL_COMPONENTS
EOF
    fi
